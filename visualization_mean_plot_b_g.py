...
array_slice = np.mean(loss_array, (2, 3))
...
im = ax.pcolormesh(g_, b_, array_slice, cmap='OrRd', norm=norm)# , interpolation='bilinear'
#ax.set_yticks(np.arange(len(b_)), labels=[round(i, 2) for i in b_])
#ax.set_xticks(np.arange(0, len(g_), 3), labels=[round(i, 2) for i in g_][::3])

ax.set_ylabel('β_2 [ps^2/m]')
ax.set_xlabel('γ [1/W/m]')


#cbar = fig.colorbar(im, orientation='vertical')
#cbar.set_label('Wartości strat', loc='center')  # Opcjonalnie, etykieta dla paska kolorów

plt.savefig(os.path.join(save_path, 'mean_plot_b_g.eps'))
plt.savefig(os.path.join(save_path, 'mean_plot_b_g.png'))
